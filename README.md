...
- **ZFS Boot Pool & Kernel Downgrade Compatibility Guard (Critical)**: Implemented a mandatory safety check detecting ZFS root boot pools configured with OpenZFS 2.4+ features (like `com.truenas:block_cloning_endian` generated by default in PVE 9.2.1+ clean installations). Since kernel 6.x's older ZFS module cannot read/write pools using these features, booting a downgraded kernel on a ZFS 2.4 root pool causes a fatal `initramfs` zfs import panic. The installer now detects this configuration and blocks kernel downgrades, preventing system bricking and guiding users to reinstall using PVE 9.1.1 or older. **(Note: This check is only active when `vgpu_unlock` is required, so native/enterprise GPU setups are unaffected.)**
...
- **Expanded Driver Support**: Added support for driver versions from v1.1 and [PTHyperdrive fork](https://github.com/PTHyperdrive/proxmox-vgpu-installer):
  ...
  - v17.x series: 17.0, 17.1, 17.3, 17.4, 17.5, 17.6 (Turing/Newer GPUs)
...
- **17.5**: 550.144.02 (Turing or newer GPUs)
- **17.4**: 550.127.06 (Turing or newer GPUs)
- **17.3**: 550.90.05 (Turing or newer GPUs)
- **17.1**: 550.54.16 (Turing or newer GPUs)
- **17.0**: 550.54.10 (Turing or newer GPUs, legacy support)

...
- **ZFS Boot Pool & Kernel Downgrade Compatibility Guard**: Prevents users on PVE 9.2+ clean installations with ZFS root filesystem from downgrading to kernel 6.x. PVE 9.2+ ZFS pools enable OpenZFS 2.4 feature tags (specifically `com.truenas:block_cloning_endian`) which are unsupported by the older ZFS module inside kernel 6.x. The script detects active features on the boot pool via `findmnt` and `zpool` and aborts before kernel downgrade, protecting the system from an unbootable state (`cannot import 'rpool': unsupported version or feature` in initramfs). **Note: This safety check only triggers when `vgpu_unlock` is actually required (i.e., `VGPU_SUPPORT="Yes"` for consumer cards), allowing native/enterprise cards to bypass this restriction.**
